...
exports.signUp = (req, res) => {
    ...
    const parkingPlace = new ParkingPlace({
        ...
        ParkingHouse.findByIdAndUpdate(data.parkingHouse, { $push: { parkingplace: data._id } }, { new: true }, (err, doc) => {
            if (err) {
...
exports.modify = (req, res) => {
    ...
    ParkingPlace.findOneAndUpdate({ _id: parkingPlaceId }, { $set: { displayName: displayName } }, { multi: true, safe: true }, function (err, parkingPlace) {
...
exports.delete = (req, res) => {
    ...
    ParkingPlace.findOneAndDelete(parkingPlaceId, function (err, parkingplace) {
        ...
        ParkingHouse.findOneAndUpdate(parkingplace.parkingHouse, { $pull: { parkingplace: parkingplace._id } }, (err, doc) => {
            if (err) {
                res.json({
                    "error-response": {
                        "error-code": 500,
