...
exports.userInfo = (req, res) => {
    User.findOne(req.params.mobileNumber).populate([{
        path: 'parkingPlace',
...
exports.userProfile = (req, res) => {
    User.findOne(req.params.mobileNumber).then(user => {
        if (!user) {
            return res.status(404).send({
...
exports.releaseParkingPlace = (req, res) => {
    ...
    User.findOneAndUpdate({ $or: [{ mobileNumber: loginAlias }, { userName: loginAlias }, { email: loginAlias }] }, { $unset: { parkingPlace: parkingPlaceID } }, { upsert: true, new: true }, function (err, user) {
        ...
        ParkingPlace.findById(parkingPlaceID, function (err, parkingPlaceFound) {
            ...
            data.update({ $unset: { owner: data._id } }, { upsert: true, new: true }, function (err, parkingPlaceFound) {
                res.status(200).send({
...
exports.setAvailableDay = (req, res) => {
    ...
    Availability.findOne({ parkingPlace: parkingPlaceID }).exec().then(data => {
        if (data) {
            ...
            Occupation.findOne({ day: avaibilityfound.day, parkingPlace: avaibilityfound.parkingPlaceID }, function (err, occupatioFound) {
                ...
                if (occupatioFound) {
                    ...
                    newOccupation.save().then(occupation => {
                        User.findOneAndUpdate({ mobileNumber: loginAlias }, { $push: { occupations: occupation } }, { upsert: true, new: true }).then(user => {
                            Availability.findOneAndUpdate({ parkingPlace: occupation.parkingPlace }, { $pull: { day: day } }, { upsert: true }).then(res.status(200).json({
...
exports.token = (req, res) => {
    ...
    User.findOne({ userName: userName }, function (err, userFound) {
        ...
        tokenToSave.save().then(data => {
            User.findOneAndUpdate({ _id: data.owner }, { $push: { token: data } }, { new: true }, function (err, userFound) {
                if (err) {
                    res.json(err);
                }
            });
    ...
    User.findOne({ $or: [{ mobileNumber: loginAlias }, { userName: loginAlias }, { email: loginAlias }] }, function (err, userFound) {
        ...
        if (data.securityQuestion == securityQuestion && data.securityAnswer != securityAnswer) {
            ...
            var mailOptions = {
                ...
                html: "Dear user, <br> You have received One Time Password. You can find it here: " + otp + "<br>Use this password to finnish your action "
            };
            const otp2 = otp;
                ...
                if (error) {
                    ...
                    bcrypt.genSalt(10, function (err, salt) {
                        ...
                        bcrypt.hash(otp2, salt, function (err, hash) {
                            ...
                            data.update({ $set: { oneTimePassword: hash, optWasUsed: false } }, { upsert: true, new: true }).then(res.end("One-time password has been sent via e-mail to user. Reset password link has been sent successfully"));
                        });
                    });
                }
            });
                            ...
                            if (response) {
                                ...
                                bcrypt.genSalt(10, function (err, salt) {
                                    ...
                                    bcrypt.hash(passwordToSave, salt, function (err, hash) {
                                        ...
                                        data.update({ $set: { password: hash, optWasUsed: true } }, { upsert: true, new: true }).then(res.status(200).json({ message: "Password was successfully changed" })).catch(err => {
                                            res.json({
                                                "error-response": {
                                                    "error-code": 500,
                                                    "error'key": 500,
